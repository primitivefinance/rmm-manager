...
describe('useKey', function () {
  ...
  describe('when the key is valid', function () {
    ...
    it('whitelists the sender', async function () {
      ...
      expect(
        await this.contracts.whitelist.isWhitelisted(this.signers[0].address)
      ).to.equal(true)
    })
    ...
    it('whitelists a specific user', async function () {
      ...
      expect(
        await this.contracts.whitelist.isWhitelisted(this.signers[1].address)
      ).to.equal(true)
    })

    it('emits the Whitelisted event', async function () {
      await expect(this.contracts.whitelist.useKey('foo', this.signers[1].address)).to.emit(
        this.contracts.whitelist,
        'Whitelisted'
    ...
    it('reverts if the user is already whitelisted', async function () {
      await this.contracts.whitelist.useKey('foo', this.signers[0].address)
      await expect(
        this.contracts.whitelist.useKey('bar', this.signers[0].address)
  ...
  describe('when the key is invalid', function () {
    it('reverts if the key is used twice', async function () {
      ...
      await this.contracts.whitelist.useKey('foo', this.signers[0].address)
      await expect(
        this.contracts.whitelist.useKey('foo', this.signers[1].address)
