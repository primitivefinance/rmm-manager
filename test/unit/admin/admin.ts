...
describe('admin', function () {
  ...
  describe('admin', function () {
    it('returns the address of the current admin', async function () {
      expect(await this.contracts.admin.admin()).to.equal(this.signers[0].address)
    })
  })

  describe('setAdmin', function () {
    it('sets a new admin if the sender is the current admin', async function () {
      await this.contracts.admin.setAdmin(this.signers[1].address)
      expect(await this.contracts.admin.admin()).to.equal(this.signers[1].address)
    })
