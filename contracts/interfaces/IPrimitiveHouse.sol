  ...
  IPrimitiveSwapCallback 
{
    ...
    // Margin
    function create(uint256 strike, uint64 sigma, uint32 time, uint256 riskyPrice) external;
    function deposit(address owner, uint delRisky, uint delStable) external;
    function withdraw(uint256 delRisky, uint256 delStable) external;
    function borrow(bytes32 poolId, address owner, uint delLiquidity, uint256 maxPremium) external;
    function allocate(bytes32 poolId, address owner, uint256 delLiquidity, bool fromMargin) external;
    function repay(bytes32 poolId, address owner, uint256 delLiquidity, bool fromMargin) external;
    // Swap
    function swap(bytes32 poolId, bool addXRemoveY, uint256 deltaOut, uint256 maxDeltaIn, bytes memory data) external;
